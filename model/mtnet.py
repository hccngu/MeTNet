...
class MTNet(utils.framework.FewShotNERModel):
    ...
    def __init__(self, word_encoder, dot=False, args=None, ignore_index=-1):
        ...
        self.att = nn.Linear(768, 768, bias=False)
        if self.args.multi_margin is True:
            if self.args.have_otherO is True:
                self.param = nn.Parameter(torch.Tensor([args.trainable_margin_init for _ in range(self.args.N+1)]))  # 8.5
            else:
                self.param = nn.Parameter(torch.Tensor([args.trainable_margin_init for _ in range(self.args.N)]))  # 8.5
        else:
            self.param = nn.Parameter(torch.Tensor([args.trainable_margin_init]))  # 8.5
        # if self.args.alpha_is_trainable:
        self.alpha = nn.Parameter(torch.Tensor([args.trainable_alpha_init]))  # 0.5
        # else:
        #     self.alpha = args.trainable_alpha_init
    ...
    def forward(self, data, model_parameters=None):
        ...
        if model_parameters is None:
            out = self.__forward_once__(data)  # [x, 768] -> [x, 256]
            out = self.drop(out)
        else:
            out = self.__forward_once_with_param__(data, model_parameters)
            out = self.drop(out)

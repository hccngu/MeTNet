...
class Sample(FewshotSampleBase):
    def __init__(self, filelines):
        ...
        for l in filelines:
            if len(l) == 2:
                filelines_new.append(l)
        self.words, self.tags = zip(*filelines_new)
    ...
    def get_tag_class(self):
        ...
        if 'O' in tag_class:
            tag_class.remove('O')
        return tag_class  # tag_class:[organization-education, B, C, ...]
...
class FewShotNERDatasetWithRandomSampling(data.Dataset):
    ...
    def __insert_sample__(self, index, sample_classes):
        '''
        get dict -> {
            'label name(organization-education)':[0, 1, ...]
            ...
        }
        '''
    ...
    def __load_data_from_file__(self, filepath):
        ...
        for line in lines:
            ...
            if line:
                ...
                classes += sample_classes  # classes:[A, B, ...]除了O以外，其他标签的顺序排列结果（有重复）
                samplelines = []
                index += 1
        if samplelines:
            sample = Sample(samplelines)
        ...
        return samples, classes  # samples列表里全是sample类的实例化， classes列表里是label names

    def __get_token_label_list__(self, sample):
        tokens = []
        labels = []
        for word, tag in zip(sample.words, sample.normalized_tags):
            word_tokens = self.tokenizer.tokenize(word)  # word就是一个个单词
            if word_tokens:
